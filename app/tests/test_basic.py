...
class BasicTests(unittest.TestCase):
    ...
    @_logger
    def test_delete_secret_without_passphrase(self):
        data = {
            'secret': 'TestSecret',
            'ttl': '1',
        }
        ...
        secret_id = s.save()
        keys_count = len(self.app.redis.keys())
        response = self.app_client.delete(url_for('secret.read_secret', secret_id=secret_id), follow_redirects=True)
        self.assertEqual(keys_count - 1, len(self.app.redis.keys()))
        self.assertEqual(response.status_code, 200)
        response = self.app_client.get(url_for('secret.read_secret', secret_id=secret_id))
